...
function enhance(){
 ...
 <section class="events-hero">
  <div class="events-hero-copy">
   ...
   <div class="events-hero-actions">
    <button class="btn primary" data-yb78-action="sprint">⚡ 60 saniye başla</button>
    <button class="btn secondary" data-yb78-action="arena">⚔️ Arena'ya git</button>
   </div>
  </div>
  <div class="events-hero-stats">
   <div class="events-stat accent"><b>${questions().length}+</b><span>soru havuzu</span></div>
   <div class="events-stat"><b>6</b><span>mini oyun</span></div>
   <div class="events-stat"><b>1 VS 1</b><span>canlı arena</span></div>
   <div class="events-stat"><b>${acc===null?'—':acc+'%'}</b><span>ortalama doğruluk</span></div>
  </div>
 </section>
 ...
 <section class="events-mode-grid">
  ...
  <article class="events-mode-card"><div class="events-mode-top"><div class="events-mode-icon">💡</div><span>İPUCU</span></div><h3>İpucu Avı</h3><p>İpuçlarını kullanarak doğru cevabı daha hızlı bul.</p><div class="events-mode-foot"><small>strateji</small><button class="btn secondary" data-yb78-game="clue">Başla →</button></div></article>
 ...
 <section class="events-arena-strip">
  <div class="events-arena-icon">⚔️</div><div class="events-arena-copy"><span>CANLI ARENA</span><h3>Gerçek rakip bul, reytingini yükselt.</h3><p>Otomatik eşleşme veya oda koduyla arkadaşınla karşılaş.</p></div><div class="events-arena-actions"><button class="btn primary" data-yb78-action="arena">Arena'yı aç</button><button class="btn secondary" data-yb78-action="social">Sosyal</button></div>
 </section>
 <div class="events-footer-note"><b>💡 Çalışma döngüsü:</b> konu kartını oku → mini oyunla pekiştir → Arena'da hızını test et.</div>
 </section>`;
 const old=Array.from(v.children);old.forEach(x=>x.remove());v.appendChild(root);
 bind(v);
}
function bind(v){
