...
(() => {
  ...
  const DEFAULT = {discovered:[],favorites:[],topicPct:{},results:[],streak:[],profile:{displayName:'Öğrenci',email:''},theme:'dark'};
  let state = loadState();
  let currentView = 'dashboard';
  let geojson = null;
  let atlas = {paths:[],bounds:null,mode:'default'};
  let selectedProvince = null;
  let quiz = null;
  let pomodoro = {seconds:25*60,total:25*60,running:false,timer:null};
  ...
  function solved(){return state.results.reduce((n,r)=>n+(Number(r.total)||0),0)}
  function correct(){return state.results.reduce((n,r)=>n+(Number(r.correct)||0),0)}
  function score(){const t=solved();return t?Math.round(correct()/t*100):0}
  ...
  function setupAuth(){
    ...
    $$('.auth-tab').forEach(b=>b.addEventListener('click',()=>{const tab=b.dataset.authTab;$$('.auth-tab').forEach(x=>x.classList.toggle('active',x===b));$('#login-wrap').classList.toggle('active',tab==='login');$('#register-wrap').classList.toggle('active',tab==='register')}));
    $('.password-toggle')?.addEventListener('click',e=>{const i=$('#'+e.currentTarget.dataset.target);i.type=i.type==='password'?'text':'password';e.currentTarget.textContent=i.type==='password'?'Göster':'Gizle'});
    $('#guest-btn').onclick=()=>{state.profile={displayName:'Misafir Öğrenci',email:''};localStorage.setItem(DEMO_SESSION_KEY,'1');saveState();showApp();toast('Misafir olarak devam ediyorsun. Verilerin bu cihazda saklanır.');};
    $('#login-form').addEventListener('submit',async e=>{e.preventDefault();const email=$('#login-email').value.trim(),password=$('#login-password').value;if(!sb){state.profile={displayName:email.split('@')[0]||'Öğrenci',email};localStorage.setItem(DEMO_SESSION_KEY,'1');saveState();showApp();return}try{const {data,error}=await sb.auth.signInWithPassword({email,password});if(error)throw error;user=data.user;await syncRemote();showApp();toast('Hoş geldin!')}catch(err){$('#login-error').textContent=authError(err)}});
    $('#register-form').addEventListener('submit',async e=>{e.preventDefault();const name=$('#register-name').value.trim(),email=$('#register-email').value.trim(),password=$('#register-password').value;if(!sb){state.profile={displayName:name,email};saveState();showApp();return}try{const {data,error}=await sb.auth.signUp({email,password,options:{data:{display_name:name}}});if(error)throw error;state.profile={displayName:name,email};saveState();if(data.session){user=data.user;showApp()}else{toast('Kayıt tamamlandı. E-postanı doğrula.')} }catch(err){$('#register-error').textContent=authError(err)}});
    $('#forgot-btn').onclick=async()=>{const email=$('#login-email').value.trim();if(!email)return toast('Önce e-posta adresini yaz.','error');if(!sb)return toast('Misafir modunda şifre sıfırlama yok.','error');try{const {error}=await sb.auth.resetPasswordForEmail(email,{redirectTo:location.origin+location.pathname});if(error)throw error;toast('Şifre sıfırlama bağlantısı gönderildi.')}catch(e){toast(authError(e),'error')}};
