...
async function record(q,correct){if(!sb||!uid||!q?.id)return;try{const r=await sb.rpc('record_learning_answer',{p_topic_id:String(q.topic||'genel'),p_correct:!!correct,p_source_id:String(q.id),p_difficulty:Math.max(1,Math.min(5,Number(q.difficultyLevel||({kolay:2,orta:3,zor:5}[q.difficulty]||3))))});if(r.error)throw r.error;if(r.data&&!r.data.duplicate)window.YB91LearningLast=r.data;await refresh()}catch(e){console.warn('YB91 record answer',e)}}
function watchQuiz(){document.addEventListener('click',e=>{const b=e.target.closest('#yb88-quiz [data-answer]');if(!b)return;setTimeout(()=>{const modal=$('#yb88-quiz .yb88-q-modal');if(!modal)return;const text=modal.querySelector('h2')?.textContent?.trim()||'';const q=(Array.isArray(window.QUESTION_BANK)?window.QUESTION_BANK:[]).find(x=>String(x.q||'').trim()===text);if(!q)return;record(q,b.classList.contains('correct'))},0)})}
