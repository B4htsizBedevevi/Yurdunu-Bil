...
function draw(svg,mode){clear(svg);if(!svg||!['lakes','rivers','mountains','plains','plateaus','mining'].includes(mode))return;const f=window.GEO_FEATURES||{};const fs=window.__YB30_GEO?.features||[];if(!fs.length)return;const p=project(bounds(fs)),g=document.createElementNS(NS,'g');g.setAttribute('class','v36-feature-layer '+mode);
if(mode==='rivers'){
...
svg.appendChild(g)}
function key(shell,mode){let k=$('.v36-map-key',shell);if(!k){k=document.createElement('div');k.className='v36-map-key';const legend=$('.legend',shell);legend?legend.after(k):shell.appendChild(k)}const labels={lakes:['<i class="lake"></i> Göl yüzeyi','İsim = göl adı'],rivers:['<i></i> Akarsu çizgisi','İsim = akarsu'],mountains:['<i class="mountain"></i> Dağ','İsim = dağ'],plains:['Ovalar','İsim = ova'],plateaus:['Platolar','İsim = plato'],mining:['Maden merkezi','İsim = maden']};const a=labels[mode];k.innerHTML=a? a.map(x=>`<span>${x}</span>`).join(''):'<span>81 il</span>'}
async function ensureGeo(){if(window.__YB30_GEO?.features?.length)return true;try{const r=await fetch('data/provinces.geojson',{cache:'no-store'});if(r.ok){window.__YB30_GEO=await r.json();return true}}catch{}return false}
function runShell(shell){if(shell.dataset.v36Bound==='1')return;shell.dataset.v36Bound='1';const tabs=$('.mode-tabs',shell);if(!tabs)return;const update=async()=>{const active=tabs.querySelector('button.active');const mode=active?.dataset.v30Mode||shell.dataset.v30Mode||'default';if(await ensureGeo())draw($('.atlas-svg',shell),mode);key(shell,mode)};update();tabs.addEventListener('click',()=>setTimeout(update,0));new MutationObserver(()=>update()).observe(tabs,{subtree:true,attributes:true,attributeFilter:['class']})}
function run(){$$('.atlas-shell').forEach(runShell)}
run();new MutationObserver(()=>{clearTimeout(window.__yb36Timer);window.__yb36Timer=setTimeout(run,120)}).observe(document.body,{subtree:true,childList:true});
})();
