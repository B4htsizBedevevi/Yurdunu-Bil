...
function safeDashboard(){
  ...
  const hasMapShell=!!v.querySelector('.atlas-card,#dash-atlas,.atlas-shell,.atlas-svg')||MAP_RX.test(text);
  const broken=v.children.length===0||hasMapShell;
  if(!broken)return;
...
function refresh(){killMap();repairProfile();safeDashboard()}
