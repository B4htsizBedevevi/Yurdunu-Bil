...
for(const raw of source){
 ...
 clean.push({...raw,id,topic,originalTopic,subtopic:String(raw.subtopic||raw.type||'genel'),difficulty:['kolay','orta','zor'].includes(raw.difficulty)?raw.difficulty:'orta',answer});seen.add(id);
}
...
window.YBQuestionPool={version:'1.0',questions:clean,total:clean.length,byTopic,byDifficulty,sourceCount:source.length,dropped:source.length-clean.length,get(topic){return topic&&topic!=='all'?clean.filter(q=>q.topic===topic):clean.slice()}};
})();