...
function eventsGuard(){
 ...
 v.querySelectorAll('.ybArena,.arena-entry,.events-arena-strip,.yb55-arena-restore,#open-arena,#open-social,[data-yb78-action="arena"],[data-yb55-arena],[data-yb55-social]').forEach(x=>x.remove());
 if(!v.querySelector('.yb86-events-page')&&window.YB86Events?.render){window.YB86Events.render()}
}
new MutationObserver(()=>setTimeout(eventsGuard,0)).observe(document.body,{childList:true,subtree:true});
window.addEventListener('load',()=>setTimeout(eventsGuard,300));
