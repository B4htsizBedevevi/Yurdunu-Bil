/* Yurdunu Bil 45.4 — premium boot + auth transition + crash recovery */
(()=>{
'use strict';
if(window.__YB45_BOOT_EXPERIENCE__)return;
window.__YB45_BOOT_EXPERIENCE__=true;
const $=(s,r=document)=>r.querySelector(s);
let overlay,recovery,timer,softTimer,loading=false,errors=[],submitting=false;
const MAX_ERRORS=4;
function ensure(){
 if(overlay)return overlay;
 overlay=document.createElement('div');
 overlay.className='yb45-boot-overlay';
 overlay.innerHTML='<div class="yb45-boot-card" role="status" aria-live="polite"><div class="yb45-boot-mark"><span>⌖</span></div><div class="yb45-boot-kicker">YURDUNU BİL • KPSS COĞRAFYA</div><h2 class="yb45-boot-title">Çalışma alanın hazırlanıyor</h2><p class="yb45-boot-text">Hesabın doğrulandı. Notların, testlerin ve ilerlemen yükleniyor.</p><div class="yb45-boot-progress"><i></i></div><div class="yb45-boot-status">Güvenli şekilde hazırlanıyor…</div></div>';
...
function show(text='Veriler hazırlanıyor…'){
 loading=true;document.body.classList.add('yb45-booting');const o=ensure();$('.yb45-boot-text',o).textContent=text;$('.yb45-boot-status',o).textContent='Bağlantı ve çalışma alanı kontrol ediliyor…';o.classList.add('is-visible');clearTimeout(timer);clearTimeout(softTimer);
 timer=setTimeout(()=>{if(appReady())hide()},12000);
 softTimer=setTimeout(()=>{if(!appReady()&&!authVisible())recoveryScreen('Uygulama beklenenden uzun sürede açıldı. Bağlantıyı kontrol edip güvenli şekilde yeniden deneyebilirsin.');else if(authVisible()&&submitting){submitting=false;setButtons(false);hide()}},10000);
}
function hide(){loading=false;submitting=false;clearTimeout(timer);clearTimeout(softTimer);document.body.classList.remove('yb45-booting');overlay?.classList.remove('is-visible');setButtons(false)}
function recoveryScreen(message){
 ...
 $('[data-yb45-dismiss]',recovery).onclick=()=>{recovery.remove();recovery=null;errors=[];if(authVisible())hide()};
}
...
function watch(){
 ...
 const obs=new MutationObserver(()=>{
   const ready=!app.classList.contains('hidden');
   const authNow=!auth.classList.contains('hidden');
   if(ready&&loading){setTimeout(hide,650);return}
   if(authNow&&!submitting&&loading){setTimeout(hide,250);return}
...
function start(){
 ...
 if(appReady())hide();
 else if(authVisible()){} else show('Yurdunu Bil başlatılıyor…');
}
