/* Yurdunu Bil — canonical boot during semantic migration. */
(()=>{
'use strict';
...
'app.js','features/games/games-core.js','arena-v1.js','v53-arena-social.js','v71-stability.js','features/library/content.js','features/library/library.js','features/library/study.js','features/library/review.js','v79-arena-matchmaking.js','v80-progress-center.js','features/library/expansion.js','features/games/events.js','features/library/deepening.js','features/questions/questions.js','v90-command-center.js','features/library/compact.js','v91-learning-bridge.js','v92-system-audit.js','v98-ui-cohesion.js','features/games/events-plus.js','v104-progress-loop.js','v108-game-progression.js','onboarding.js','notifications.js','flashcards.js','leaderboard.js','features/ui/effects.js'];
...
(async()=>{try{for(const file of files)await load(file);root.register?.('boot',{ready:true,modules:files.length,migration:'semantic-library-games'});document.documentElement.classList.add('yb-ready');window.dispatchEvent(new CustomEvent('yb:ready',{detail:{modules:files.length}}));}catch(error){console.error(error);root.register?.('boot',{ready:false,error:String(error)});document.documentElement.classList.add('yb-boot-error');const toast=document.getElementById('toast-root');if(toast)toast.textContent='Uygulama modüllerinden biri yüklenemedi. Sayfayı yenileyin.'}})();
})();
