...
(() => {
  ...
  async function doLogin(email, password) {
    if (sb) {
      ...
      /*
       * Auth başarılıysa kullanıcıyı bekletme.
       * Veri senkronizasyonu başarısız olsa bile uygulama açılmalı;
       * local state çalışmaya devam eder ve senkronizasyon tekrar denenebilir.
       */
      showAppScreen();
      saveState();
      ...
      try {
        ...
        console.warn("Giriş sonrası veri senkronizasyonu başarısız:", syncError);
        toast("Giriş başarılı. Veriler daha sonra senkronize edilecek.", "info");
      }
  ...
  async function doRegister(name, email, password) {
    if (sb) {
      ...
      if (data.session && data.user) {
        ...
        toast(
          "Hesabın hazır! Çalışmaya başla. 🚀",
          "success"
        );
      } else {
        toast(
          "Kayıt başarılı! E-postanı doğrulayıp giriş yap.",
          "success"
        );
  ...
  async function doForgotPassword() {
    ...
    if (!sb) {
      toast(
        "Demo modunda şifre sıfırlama kullanılamaz.",
        "info"
      );
      return;
    }
  ...
  async function doGoogleLogin() {
    if (!sb) {
      toast(
        "Google girişi gerçek Supabase bağlantısı gerektirir.",
        "info"
      );
      return;
    }
        ...
        <div class="quiz-card-body">
          ...
          <div class="quiz-options-v8">
            ${optionsSafe(question.options).map((option,i)=>{
              ...
              const stateIcon = quizSession.submitted ? (i === correctIndex ? '✓' : (i === selected ? '×' : '')) : '';
    ...
    $("notif-btn")
      ?.addEventListener(
        ...
        event => {
          ...
          toast(
            "Bugünkü hedef: 3 il keşfet + 1 test çöz. Hadi! 🎯",
            "info"
          );
      ...
      async (
        ...
        if (
          ...
          try {
            ...
            console.warn("Oturum verileri senkronize edilemedi:", syncError);
            toast("Oturum açıldı. Veriler daha sonra senkronize edilecek.", "info");
          }
  ...
  async function boot() {
      ...
      try {
        ...
        if (
          ...
          try {
            ...
            console.warn("Kayıtlı oturum verileri senkronize edilemedi:", syncError);
          }
    ...
    showAuthScreen(getRequestedAuthMode());
  }
