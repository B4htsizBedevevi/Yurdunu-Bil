/* Yurdunu Bil 56.1 — compact Events hub + clean information ownership */
(()=>{'use strict';
...
function cleanLegacyStats(v){
 if(!v)return;
 $$('.yb52-dashboard,.yb52-arena-card,.yb54-section-strip,.yb51-summary,.yb51-stats,.yb51-score,.yb51-result-summary',v).forEach(e=>e.remove());
 $$('*',v).forEach(e=>{const t=(e.textContent||'').replace(/\s+/g,' ').trim();if(/^\d+\s*XP\s*[•·]\s*\d+\s*cevap\s*[•·]\s*\d+\s*doğru$/i.test(t)&&e.children.length<4)e.remove()});
}
function showEvents(){if(arenaOpen)return;const v=$('#view-events');if(!v||!v.classList.contains('active'))return;cleanLegacyStats(v);if(v.querySelector('.yb56-events-hub')){bind(v);return}v.innerHTML=hub();bind(v)}
...
function onBack(e){const b=e.target.closest('[data-yb56-back-events]');if(!b)return;arenaOpen=false;window.__YB56_ARENA_OPEN__=false;const nav=$('[data-view="events"]');if(nav)nav.click();else showEvents()}
