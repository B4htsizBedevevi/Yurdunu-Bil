...
const checks=[
  ['Ana sayfa konu kartları', 'features/home/home.js', [/data90topic/, /window\.navigate\?\.\('library'\)/]],
  ...
  ['Arena navigasyonu', 'features/ui/navigation.js', [/view==='arena'/, /YBArena/]],
...
for(const [name,file,patterns] of checks){
  let text='';
  try{text=read(file)}catch(e){console.error(`✖ ${name}: ${file} okunamadı`);failed++;continue}
  const missing=patterns.filter(p=>!p.test(text));
  if(missing.length){console.error(`✖ ${name}: ${file} içinde ${missing.length} sözleşme eksik.`);failed++}
  else console.log(`✓ ${name}`);
}
const index=read('index.html');
const views=[...index.matchAll(/data-view="([^"]+)"/g)].map(m=>m[1]);
for(const v of new Set(views)){
  if(!['home','library','events','settings','arena','map'].includes(v)){
    console.error(`✖ Tanımsız navigasyon rotası: ${v}`);failed++;
  }
}
const css=read('styles/app.css');
const stale=(css.match(/\?v=99\.0\.5/g)||[]).length;
if(stale){console.error(`✖ styles/app.css içinde ${stale} adet eski 99.0.5 cache referansı kaldı.`);failed++}else console.log('✓ CSS cache sürümü güncel');
if(failed){console.error(`\n✖ Etkileşim denetimi ${failed} hata ile başarısız.`);process.exit(1)}
console.log('✓ Etkileşim sözleşmeleri temiz: ana sayfa → konu → kütüphane → soru/oyun/Arena akışları doğrulandı.');
