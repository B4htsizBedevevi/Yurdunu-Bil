...
async function fetchBoard(tab){
 ...
 const rows=(profiles||[]).map(p=>{
   const r=statMap[p.id]||{};
   const name=String(p.display_name||'').trim();
   const isArena=tab==='arena';
   return{
     id:p.id,username:name,avatar:p.avatar_emoji||'🧭',
     xp:Number(isArena?(r.rating??1000):(r.xp??0))||0,
     answers:Number(isArena?(r.matches??0):(r.total_questions??0))||0,
     correct:Number(isArena?(r.wins??0):(r.total_correct??0))||0,
     streak:Number(r.best_streak??0)||0,
     isYou:p.id===uid
   };
 }).filter(r=>r.username.length>=2);
 rows.sort((a,b)=>b.xp-a.xp||b.correct-a.correct||a.username.localeCompare(b.username,'tr'));
...
function render(list,rows,tab,total){
 ...
 const foot=list.parentElement?.querySelector('.yb-lb-foot span');if(foot)foot.textContent=`Canlı sıralama · ${total} kayıtlı oyuncu`;
}
