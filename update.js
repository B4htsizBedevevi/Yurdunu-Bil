/* Yurdunu Bil 41 — deployment-aware update monitor */
(()=>{
'use strict';
...
function show(remote,msg){if($('.'+css))return;const n=document.createElement('aside');n.className=css;n.innerHTML=`<div class="yb41-update-kicker">🚀 YENİ SÜRÜM DAĞITILIYOR</div><h3>Yurdunu Bil ${esc(remote)}</h3><p>${esc(msg||'Yeni harita, içerik ve hata düzeltmeleri hazırlanıyor. Render dağıtımı tamamlanınca bu sayfa otomatik yenilenecek.')}</p><div class="yb41-update-status"><span class="yb41-update-dot"></span><b data-yb41-update-state>Dağıtım bekleniyor…</b></div><div class="yb41-update-actions"><button type="button" data-yb41-now>Şimdi kontrol et</button><button type="button" data-yb41-close>Daha sonra</button></div>`;document.body.appendChild(n);n.querySelector('[data-yb41-now]').onclick=()=>poll(remote,true);n.querySelector('[data-yb41-close]').onclick=()=>n.remove()}
